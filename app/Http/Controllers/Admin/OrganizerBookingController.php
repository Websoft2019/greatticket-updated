...
use App\Mail\MakeReservedPaymentMail;
...
class OrganizerBookingController extends Controller
{
    ...
    public function store(Request $request, TicketBookingService $ticketBookingService)
    {
        ...
        $rules = [
                    'booking_type' => ['required', Rule::in(['normal', 'complementary', 'reserved'])],
                    'customer_name' => ['required_if:booking_type,reserved', 'string', 'nullable'],
                    'customer_email' => ['required_if:booking_type,reserved', 'email', 'nullable'],
                    'package_id' => ['required', 'exists:packages,id'],
                    'quantity' => ['required', 'integer', 'min:1'],
                    'coupon_code' => ['nullable', 'string'],
                ];

        // Conditionally require attendees only if NOT reserved
        if ($request->booking_type !== 'reserved') {
            $rules['attendees'] = ['required', 'array'];
            $rules['attendees.*'] = ['required', 'string'];
        }
        ...
        try {
            ...
            $order = Order::create([
                ...
                'paymentmethod' => $request->booking_type == 'normal' ? 'Cash on counter' : ($request->booking_type == 'complementary' ? 'complementary' : 'reserved'),
            ...
            if ($request->booking_type == "reserved") {
                $purchaseDetails = [
                    ...
                    'seats' => $order->orderPackages()->first()->ticketUsers
                        ->filter(function ($user) {
                            return $user->seat_id;
                        })
                        ->map(function ($user) {
                            return $user->seat->row_label . $user->seat->seat_number;
                        })
                        ->values()
                        ->toArray(),
                    'amount' => $order->grandtotal,
                    'deadline' => $order->expires_at,
                    'paymentUrl' => route('payment.reserved', ['order' => $order->qr_code])
                ];

                Mail::to($order->email)
                    // ->bcc(['greatticketmy@gmail.com'])
                    ->send(new MakeReservedPaymentMail($purchaseDetails));
            } else {
    ...
    public function downloadPdf(Order $order)
    {
        ...
        $purchaseDetails = [
            ...
            'payment_status' => $order->paymentstatus == 'Y' ? 'Paid' : ($order->paymentstatus == 'R' ? 'Reserved' : 'Pending'),
